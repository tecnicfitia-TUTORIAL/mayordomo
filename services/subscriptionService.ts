...
export const SubscriptionService = {
  ...
  getCurrentUserTier: async (userId?: string): Promise<SubscriptionTier> => {
    ...
    // Valor por defecto para desarrollo (Hardcoded)
    // FIX: Se establece en FREE para que los usuarios nuevos no vean el panel de Admin/Gobernante por defecto.
    const HARDCODED_DEV_TIER = SubscriptionTier.FREE; 

    // Simula latencia
    await new Promise(resolve => setTimeout(resolve, 500));

    return HARDCODED_DEV_TIER;
  },
